...
# Getting Started
You've been provided some starter code (scaffolding) in `/hdl` to help you get
started. The intended microarchitecture of this scaffolding is shown in
figure 1. Note that this scaffolding will simulate cleanly throughout CP1,
though it may be prone to throwing lint/synthesis errors and warnings until you
finish writing all the logic. All the lint and synthesis errors should be
resolved by the end of CP1, while the warnings should all be addressed by the
end of CP2. Refer to [README.md](./README.md) for our policy on errors and
warnings.

<p align="center">
  <img src="doc/images/scaffolding.svg"/>
  <p align="center">Figure 1: Processor block diagram. This is what the scaffolding follows.</p>
</p>
...
copy over any parts of that provided code you deem re-useable. We have provided
module scaffolding to aid with the completion of checkpoint 1. While you are
free to make changes or start from scratch, we hope this outline will make it
easier to implement the basic (CP1) structure of the pipeline.
...
`n_port_pipeline_memory_32_w_mask` in `hvl/common/top_tb.svh` with `MAGIC(1)`
instead of `MAGIC(0)`.
...
We recommend *global stall* as the easiest method of dealing with memory
response delays. If the processor is waiting on either `imem` or `dmem` to
respond, the entire pipeline will stall.
...
checkpoint.  All you need to do is to fill out `hvl/common/rvfi_reference.json`
...
A Python script will sanitize this file and generate a `.svh` file, sourced in
`top_tb.svh`.  Your hierarchical reference should start with `dut.`, as
specified in `top_tb.svh`.
...
permitted in `hvl/common/rvfi_reference.json` and other HVL files. If you find
...
certain classes of issues much faster than writing directed test vectors. If you
did the optional assignment in `mp_verif`, that can be reused to test your
pipeline!

To start using `randinst.svh` and `instr_cg.svh` from `mp_verif` to verify your
pipelined processor, you must port `mp_verif/cpu/hvl/vcs/random_tb.sv` to work
...
`n_port_pipeline_memory_32_w_mask.sv`.
...
Make sure that the `n_port_pipeline_memory_32_w_mask` instantiation is commented
